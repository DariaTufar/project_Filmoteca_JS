...
export default class ServerRequest { // При ініціалізації можна передавати базову урлу, та обєкт конфігурацій
    ...
    createPagination(ref) {
        ...
        if (this.page > 1) { //показывает следующую страницу если значение пейдж больше чем один
            liTag += `<li class="btn prev"><span><i class="fas fa-angle-left"></i> &#171;</span></li>`;
        }
        if (this.page > 2) { //если значение пейдж меньше чем 2 - тогда добавь один после предыдущей кнопки
            liTag += `<li class="first numb"><span>1</span></li>`;
            if (this.page > 3) { //если значение пейдж больше чем 3 - тогда добавь ... полсе первой лишки или пейдж
                liTag += `<li class="dots"><span>...</span></li>`;
            }
        }

        if (this.page == this.totalPages) { //сколько страниц или лишек показывается перед текущей Ли
            beforePage = beforePage - 2;
        } else if (this.page == this.totalPages - 1) {
            beforePage = beforePage - 1;
        }

        if (this.page == 1) { //сколько страниц или лишек показывается после текущей Ли
            afterPage = afterPage + 2;
        } else if (this.page == 2) {
            afterPage = afterPage + 1;
        }
        for (let plength = beforePage; plength <= afterPage; plength += 1) {
            if (plength > this.totalPages) { //если длина пейдж больше чем тоталпейдж тогда продолжай
                continue;
            }
            if (plength == 0) {
                plength = plength + 1;
        ...
        if (this.page < this.totalPages - 1) {
            if (this.page < this.totalPages - 2) {//рендерит разметку троиточие если страниц меньше чем две
                liTag += `<li class="dots"><span>...</span></li>`;
            }
            liTag += `<li class="last numb"><span>${this.totalPages}</span></li>`;
        }
        if (this.page < this.totalPages) { //показывает следующую кнопку если значени страницы меньше 20
